...
#define MIC_LEVEL_LOG_INTERVAL_MS 500
#define MIC_I2S_USE_RIGHT_SLOT 1
...
static esp_err_t i2s_init(i2s_port_t i2s_num, uint32_t sample_rate, int channel_format, int bits_per_chan)
{
    ...
    ESP_LOGI(TAG, "I2S mic slot: %s", MIC_I2S_USE_RIGHT_SLOT ? "RIGHT" : "LEFT");
...
static void feed_handler(AppSpeech *self)
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < samp_len; ++i)
        {
            ...
            uint32_t abs_sample = (i2s_buff[i] >= 0) ? (uint32_t)i2s_buff[i] : (uint32_t)(-i2s_buff[i]);
            level_abs_acc += abs_sample;
            level_sample_count++;
        ...
        if ((now_tick - last_level_log_tick) >= pdMS_TO_TICKS(MIC_LEVEL_LOG_INTERVAL_MS)) {
            uint32_t avg = level_sample_count ? (uint32_t)(level_abs_acc / level_sample_count) : 0;
            ESP_LOGI(TAG, "MIC level avg=%u peak=%u", (unsigned int)avg, (unsigned int)level_peak);
    ...
    while (true)
    {
        ...
        if (self->detected) {
            ...
            if (mn_state == ESP_MN_STATE_DETECTING) {
                ...
                if (play_Music_Flag) {
                    play_Music_Flag = 0;
                    ESP_LOGI(TAG, "Music command captured (no app handler wired in starter)");
                }
            }
        }
    }
